...
A skill that permits implicit invocation, so the agent can fire it autonomously — and the human can still type its name, so model-invocation always _includes_ user reach. In Codex, the `SKILL.md` **description** remains required; implicit reach is controlled by `agents/openai.yaml` policy when available. Model-invoked skills pay **context load** through their routing metadata, so their descriptions must be compact and trigger-rich. Pick model-invocation only when the agent must reach the skill on its own, or another skill must reliably reach it.

_Avoid_: ability, tool, capability

### User-Invoked

A skill intended for explicit human invocation. In Codex, keep the required `name` and **description** in `SKILL.md`, then set `policy.allow_implicit_invocation: false` in `agents/openai.yaml` when the surface supports it. This trades autonomous discovery for lower invocation risk and human control; still keep the description short because some surfaces may list explicit-only skills.

_Avoid_: procedure, workflow, command

### Description

The skill's machine-readable trigger and top-level **context pointer**. Codex requires it in `SKILL.md`; invocation policy lives separately when `agents/openai.yaml` is available. Because descriptions can be shown in skill lists and drive implicit matching, every word must earn its place.
...
The cost a skill imposes on the agent's context window through always-visible routing metadata and any loaded body/reference text. Descriptions are the highest-leverage pruning target because they are seen before a skill is selected.
...
A **user-invoked** skill whose job is to point at other procedure skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It cures **cognitive load** when explicit workflows multiply.
...
**Reference** that lives outside the skill body — a plain linked file with no trigger of its own. It is the home for shared material that needn't fire independently and should not bloat multiple `SKILL.md` files.
